--1) Используйте в своем курсоре опцию SCROLL, перейдите на последнюю запись в таблице, затем перейдите на 15 запись,
--с 15 на 7, а с 7 на 2 и затем на 1.
--Двигайтесь по курсору назад с помощью FETCH/MOVE.
...
begin;
declare c_pr scroll cursor for select * from products;
move last from c_pr;
fetch last from c_pr;
move backward 4 from c_pr;
fetch prior from c_pr;
move backward 7 from c_pr;
fetch prior from c_pr;
move backward 4 from c_pr;
fetch prior from c_pr;
